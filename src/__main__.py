...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Call Me Maybe - Constrained Decoding Pipeline"
    )
    parser.add_argument(
    ...
    try:
        ...
        print(
            ...
            "contains malformed JSON markup."
        )
        return
    ...
    for idx, item in enumerate(test_cases, start=1):
        ...
        print(
            f"[{idx}/{len(test_cases)}] Generating constrained "
            f"schema call for: '{prompt_string}'"
        )
        raw_json_output = generate_json(
            model=model, prompt=prompt_string, schemas=schemas
        )
        try:
            ...
            print(
                "Warning: Output string failed "
                f"structural parsing checks: {parse_error}"
            )
